...
//    final int maxHeightBst = maxHeightBst(binarySearchTree.root, 1);
    // System.out.println(binarySearchTree);
    //System.out.println(binarySearchTree.lookUp(15));
    //binarySearchTree.remove(20);
    //System.out.println(binarySearchTree);
    //System.out.println(binarySearchTree.breadthFirstSearch());
    final List<int[]> bfsLevelOrderasAnArray = bfsLevelOrderasAnArray(binarySearchTree.root);
    System.out.println("bfsLevelOrderasAnArray [ " + bfsLevelOrderasAnArray.stream().flatMapToInt(Arrays::stream).boxed().collect(Collectors.toList()) + " ]");
  ...
  // Time = O(M) M = no of nodes
  // Space = O(N/2)  = O(N) dropping as 2 is constant=
  // Basically, the total nmo of nodes in your binary tree
  // divide by 2 will give you the leaf nodes if it is a complete binary tree
  // and those leaf nodes size will be the space complexity of
  // this code as our queue shall hold at any given time the total no of nodes ata given level.
//if want to return levle order traversal in serialized
// nodes.stream().flatMapToInt(Arrays::stream).boxed().collect(Collectors.toList())
  public static List<int[]> bfsLevelOrderasAnArray(Node node) {
    List<int[]> nodes = new ArrayList<>(); // O(N)
    Queue<Node> queue = new LinkedList<>(); //O(N/2) = O(N)
    queue.add(node);
    while (queue.size() > 0) {
      int levelCounter = queue.size();
      int count = 0;
      int[] currentLevelValues = new int[levelCounter];
      while (count < levelCounter) {
        final Node qNode = queue.remove();
        currentLevelValues[count] = qNode.value;
        if (qNode.left != null) {
  ...
  public static int maxHeightBst(Node node, int count) {
    if (node == null) {
      return count;
    }
    count++;
    return Math.max(maxHeightBst(node.left, count), maxHeightBst(node.right, count));
  }
